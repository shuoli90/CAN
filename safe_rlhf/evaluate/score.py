...
def batch_scoring(
    ...
    pair_ids = to_device(batch[0], args.device).to(torch.int64)
    dist.barrier()
    
    attention_mask = torch.logical_and(
        pair_ids.not_equal(reward_tokenizer.pad_token_id),
        pair_ids.not_equal(reward_tokenizer.unk_token_id),
    )
    ...
    reward_score = torch.hstack((pair_ids, reward_score, cost_score)) # create the demo-score pairs
...
def main() -> None:  # pylint: disable=too-many-locals,too-many-statements
    ...
    output_file_path = args.input_dir+f'/{model_name}_{args.total_generate_size}_output_ids_new2.pt' # for test purpuse
    ...
    output_ids = torch.load(output_file_path, map_location='cpu')
    if os.path.exists(args.output_dir+f'/{model_name}_cost_reward_scores_new2_v1.pt'):
        res = torch.load(args.output_dir+f'/{model_name}_cost_reward_scores_new2_v1.pt')
        length = res.size(0)
        output_ids = output_ids[length:]
    else:
        res = None
    output_ids = output_ids.view(-1, args.max_length * 2)[:, :args.max_length]
    ...
    for i, batch in enumerate(
        ...
        if is_main_process():
            scores = scores.view(1, -1, args.max_length + 2)
            if i == 0 and res is None:
                res = scores
            else:
                res = torch.cat((res, scores), dim=0)

            if i % 20 == 0 or res_size >= args.total_generate_size: # Save intermittently (e.g., every 2 iterations)
                torch.save(res, args.output_dir+f'/{model_name}_cost_reward_scores_new2_v1.pt')
