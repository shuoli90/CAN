...
if __name__ == '__main__':
    ...
    if script_args.lamb is None:
        ...
        safety_0_log_probs = torch.load('safety_0_log_probs_train.pt')
        safety_1_log_probs = torch.load('safety_1_log_probs_train.pt')
        # load in helpful log probs
        helpful_0_log_probs = torch.load('help_0_log_probs_train.pt')
        helpful_1_log_probs = torch.load('help_1_log_probs_train.pt')
        # load in ref log probs
        ref_0_log_probs = torch.load('ref_0_log_probs_train.pt')
        ref_1_log_probs = torch.load('ref_1_log_probs_train.pt')

        dataset = load_dataset(
            "PKU-Alignment/PKU-SafeRLHF-30K", split='train'
            )

        dataset = dataset.add_column('safety_0', safety_0_log_probs.tolist())
        dataset = dataset.add_column('safety_1', safety_1_log_probs.tolist())
        dataset = dataset.add_column('help_0', helpful_0_log_probs.tolist())
        dataset = dataset.add_column('help_1', helpful_1_log_probs.tolist())
        dataset = dataset.add_column('ref_0', ref_0_log_probs.tolist())
        dataset = dataset.add_column('ref_1', ref_1_log_probs.tolist())
        ds = dataset.train_test_split(test_size=0.1,)
        trainset = ds['train']
        evalset = ds['test']
        evalset = evalset.select(range(0, int(len(evalset)/2.0)))
        ...
        def return_prompt_and_responses(samples):
            ...
            safety_0 = torch.tensor(samples['safety_0'])
            safety_1 = torch.tensor(samples['safety_1'])
            helpful_0 = torch.tensor(samples['help_0'])
            helpful_1 = torch.tensor(samples['help_1'])
            ref_0 = torch.tensor(samples['ref_0'])
            ref_1 = torch.tensor(samples['ref_1'])

            kl_r_0 = helpful_0 - ref_0
            kl_g_0 = safety_0 - ref_0
            kl_r_1 = helpful_1 - ref_1
            kl_g_1 = safety_1 - ref_1

            weight_diff = (kl_r_1-kl_r_0) + (kl_g_1-kl_g_0)*script_args.lamb
            probs = torch.sigmoid(weight_diff)
            indices = torch.bernoulli(probs).long().squeeze()

            chosen = [response_1[i] if index == 1 else response_0[i] for i, index in enumerate(indices)]
            rejected = [response_0[i] if index == 1 else response_1[i] for i, index in enumerate(indices)]
